locals {
  ...
  # external_services_storage_exists = fileexists("${path.module}/modules/storage/data/config.json") || local.external_services_storage_config != null
  ...
  get_storage_config = fileexists("${path.module}/modules/storage/data/config.json") ? jsondecode(file("${path.module}/modules/storage/data/config.json")) : null

  #external_storage_exists = fileexists("${path.module}/modules/storage/data/config.json") || local.external_storage_config != null
  ...
  #hpcc_namespace = local.hpcc_namespace.existing_namespace != null ? local.hpcc_namespace.existing_namespace : local.hpcc_namespace.create_namespace == true ? kubernetes_namespace.hpcc[0].metadata[0].name : fileexists("${path.module}/logging/data/hpcc_namespace.txt") ? file("${path.module}/logging/data/hpcc_namespace.txt") : "default"
